...
plot_trace_sim <- function(outputfull) {
    require(posterior)
    require(bayesplot)
    require(ggdist)
...
plot_abkinetics_sim <- function(outputfull) {
    require(posterior)
    require(bayesplot)
    require(ggdist)
...
plot_exp_rec_sim <- function(outputfull) {
...
plot_exp_times_rec_sim <- function(outputfull) {
...
plot_inf_rec_sim <- function(outputfull) {
...
plot_cop_rec_sim <- function(outputfull) {
...
postprocessFigs_sim <- function(modelname, modelname_sim, obs_er, n_chains) {
   # modelname <- "inferExp"
   # modelname_sim <-  "cesNoCOP_notd"
    #obs_er <- "0.3"
   # n_chains <- 4

    postprocess_run_sim( modelname, modelname_sim, obs_er, n_chains)
    outputfull <- readRDS(file = here::here("outputs", "fits", modelname_sim, modelname, paste0("pp_obs_", obs_er, ".RDS")))

    plot_trace_sim(outputfull)
    plot_abkinetic_sim(outputfull)
    plot_inf_rec_sim(outputfull)
    plot_exp_rec_sim(outputfull)
    plot_exp_times_rec_sim(outputfull)
    plot_cop_rec_sim(outputfull)
}
