...
plotPostFigsSim <- function(model_summary, sim_model, sim_res, save_info) {
    ...
    plot_titre_obs_sim(model_summary, file_path)
    plot_titre_exp_sim(model_summary, sim_model, sim_res, file_path)
    plot_cop_rec_sim(model_summary, file_path)
    plot_abkinetics_trajectories_sim(model_summary, sim_model, sim_res, file_path)
    plot_inf_rec_sim(model_summary, sim_model, sim_res, file_path)
    plot_exp_times_rec_sim(model_summary, sim_model, sim_res, file_path)
    plot_abkinetics_trajectories_ind_sim(model_summary, sim_model, sim_res, file_path)
...
plot_exp_times_rec_sim <- function(model_summary, sim_model, sim_res, file_path) {


   # sim_res$biomarker_states %>% filter(i == 124)
   # sim_res$observed_biomarker_states %>%  filter(i == 124)
    ...
    figB <- dataplt %>% mutate(id = factor(id, levels = pid_order1)) %>% filter(inf == "Infected") %>% 
        ggplot() + 
            ...
            geom_point(aes(y = id, x = inf_time_sim), shape = 17, color = "red", size = 1.5, alpha = 0.8) + 
   ...
   datasets <- fit_states %>% filter(inf_time > -1, inf_ind == 1)  %>% arrange(sample) %>% 
    ...
    df_data_t <- df_data %>% mutate(known_inf = 0) %>% mutate(known_inf = ifelse(known_inf == 0, "Not known", "Known"))
# Plotting
    figC <- df_data_t %>% ggplot() +
    geom_histogram(data = dataplt, aes(x = inf_time_sim), color = "white", size = 1.5) +
  ...
  ggsave(here::here(file_path, "exposure_time_recov.png"), height = 10, width = 10)
    ...
    dataplt_error <- dataplt %>% mutate(id = factor(id, levels = pid_order1)) %>% filter(inf == "Infected") %>% 
        mutate(across(inf_time:.upper, ~.x - inf_time_sim)) %>% arrange(inf_time)
...
plot_titre_obs_sim <- function(model_summary, file_path) {
...
plot_titre_exp_sim <- function(model_summary, sim_model, sim_res, file_path) {
...
plot_abkinetics_trajectories_sim <- function(model_summary,  sim_model, sim_res, file_path) {
   ...
   # (p1 + p2) / (p3 )
    ...
    p2 <- traj_post_summ %>%  
        ggplot() + 
        geom_line(data = traj_true, aes(y = value, x = time, group = sim, color = "red"),  size = 0.4, alpha = 0.4) + 
        geom_ribbon(aes(ymin = .lower, ymax = .upper, x = time), fill = "gray", size = 3, alpha = 0.5) + 
        geom_line(aes(y = value, x = time, color = "grayblue"), size = 2) + 
...
plot_inf_rec_sim <- function(model_summary,  sim_model, sim_res, file_path) {
    ...
    no_inf_sim <- sim_states %>% select(id, inf_num) %>% pull(inf_num) %>% sum
...
plot_cop_rec_sim <- function(model_summary, file_path) {
    ...
    cop_exp_sum_plot %>% ggplot() + 
        ...
        labs(x = "Initial titre", y = "Posterior probability of infection") + 
         geom_linerange(data = cop_exp_sum_plot,
            aes(y = inf_post, xmin = .lower, xmax = .upper), size = 1) + 
        geom_point(data = cop_exp_sum_plot, aes(x = titre_val, y = inf_post)) + 
        theme_bw()
    ggsave(here::here(file_path, "cop_recov.png"), height = 10, width = 10)
