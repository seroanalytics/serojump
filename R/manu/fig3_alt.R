...
p1 <- recover_fourfold_s %>%
  mutate(result = ifelse(inferred_4_fold, "Detected", "Missed")) %>%
  mutate(result = factor(result, levels = c("Missed", "Detected"))) %>%
  ggplot(aes(x = 1, fill = result)) +
  geom_bar(position = "fill", width = 0.3, alpha = 0.8) +
    geom_text(x = 1, y = sens_4_s + 0.05, label = paste0("Sensitivity: ", round(sens_4_s, 2)), size= 5) + 
  scale_y_continuous(labels = scales::percent) +
  scale_fill_manual(values = c("Detected" = "#6DBE45", "Missed" = "#E74C3C")) +
  theme_bw() + 
  labs(x = NULL, y = "Proportion of PCR-confirmed infections", title = "Four-fold rise (spike)", fill = "") +
...
p2 <- recover_fourfold_n %>%
  ...
  ggplot(aes(x = 1, fill = result)) +
  geom_bar(position = "fill", width = 0.3, alpha = 0.8) +
    geom_text(x = 1, y = sens_4_n + 0.05, label = paste0("Sensitivity: ", round(sens_4_n, 2)), size= 5) + 
  scale_y_continuous(labels = scales::percent) +
  scale_fill_manual(values = c("Detected" = "#6DBE45", "Missed" = "#E74C3C")) +
    theme_bw() + 
  labs(x = NULL, y = "Proportion of PCR-confirmed infections", title = "Four-fold rise (NCP)", fill = "") +
  theme(axis.text.x = element_blank(), axis.ticks.x = element_blank(), axis.title.y = element_blank(), axis.text.y = element_blank())

sens_p_s <- mean(recover_seropos_s$true_pos)

p3 <- recover_seropos_s %>%
    mutate(result = ifelse(sero_pos, "Detected", "Missed")) %>%
  mutate(result = factor(result, levels = c("Missed", "Detected"))) %>%
    ggplot(aes(x = 1, fill = result)) +
    geom_bar(position = "fill", width = 0.3, alpha = 0.8) +
    geom_text(x = 1, y = sens_p_s + 0.05, label = paste0("Sensitivity: ", round(sens_p_s, 2)), size= 5) + 
    scale_y_continuous(labels = scales::percent) +
    scale_fill_manual(values = c("Detected" = "#6DBE45", "Missed" = "#E74C3C")) +
    theme_bw() + 
    labs(x = NULL, y = "Proportion of PCR-confirmed infections", title = "Seropos threshold (spike)", fill = "") +
    theme(axis.text.x = element_blank(), axis.ticks.x = element_blank(), axis.title.y = element_blank(), axis.text.y = element_blank())
...
p4 <- recover_seropos_n %>%
  ...
  mutate(result = factor(result, levels = c("Missed", "Detected"))) %>%
    ggplot(aes(x = 1, fill = result)) +
    geom_bar(position = "fill", width = 0.3, alpha = 0.8) +
    geom_text(x = 1, y = sens_p_n + 0.05, label = paste0("Sensitivity: ", round(sens_p_n, 2)), size= 5) + 
    scale_y_continuous(labels = scales::percent) +
    scale_fill_manual(values = c("Detected" = "#6DBE45", "Missed" = "#E74C3C")) +
      theme_bw() + 
    labs(x = NULL, y = "Proportion of PCR-confirmed infections", title = "Seropos threshold (NCP)", fill = "") +
    theme(axis.text.x = element_blank(), axis.ticks.x = element_blank(), axis.title.y = element_blank(), axis.text.y = element_blank())
...
p5 <- recover_serojump %>%
# Assuming your data frame is called df
  mutate(result = ifelse(true_pos, "Detected", "Missed")) %>%
  mutate(result = factor(result, levels = c("Missed", "Detected"))) %>%
  ggplot(aes(x = 1, fill = result)) +
  geom_bar(position = "fill", width = 0.3, alpha = 0.8) +
    geom_text(x = 1, y = sens_sj + 0.05, label = paste0("Sensitivity: ", round(sens_sj, 2)), size= 5) + 
  scale_y_continuous(labels = scales::percent) +
  scale_fill_manual(values = c("Detected" = "#6DBE45", "Missed" = "#E74C3C")) +
    theme_bw() + 
  labs(x = NULL, y = "Proportion of PCR-confirmed infections", title = "`serojump`", fill = "") +
  theme(axis.text.x = element_blank(), axis.ticks.x = element_blank(), axis.title.y = element_blank(), axis.text.y = element_blank())

p0 <- p1 + p2 + p3 + p4 + p5 + plot_layout(guides = "collect", width = c(1,1,1,1,1)) + plot_annotation(tag_levels = "A") &  
  theme(
    legend.position = "bottom",
    legend.text = element_text(size = 12),      # increase legend label size
    legend.title = element_text(size = 13)      # optional: increase title size
  )
ggsave(here::here("outputs", "figs", "fig3_alt.png"), width = 15, height = 7)
ggsave(here::here("outputs", "figs", "fig3_alt.tiff"), width = 15, height = 7)
