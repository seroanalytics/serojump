...
p2 <- df_outputs_cop %>% 
    ...
    labs(x = "Normalised titre", y = "Protection (%)", color = "Biomarker", fill = "Biomarker") + 
    scale_x_continuous(
    ...
    ggtitle("E. Fitted curves for aCOP")
...
p3 <- df_outputs_cop %>% 
    ...
    ggtitle("F. Fitted curves for rCOP")
...
p1A <- df_outputs_cop %>% filter(biomarker == "spike") %>%
    ...
    scale_x_continuous(breaks = seq(0, 1, length.out = 10), labels = labels_x_spike) + 
    labs(x = "Log titre", y = "Posterior probability of infection", color = "Biomarker", fill = "Biomarker") + 
    ggtitle("Fitted curves for infection risk")
...
p2 <- df_outputs_cop %>% 
    ...
    ggtitle("Fitted curves for aCOP")

p3 <- df_outputs_cop %>% 
    ggplot() + stat_lineribbon(aes(x = x_new, y = y_hat_rel, group = biomarker, color = biomarker, fill = biomarker), .width = 0.01,alpha = 1, size = 2) + theme_bw()+ 
    scale_color_manual(values = c("#003741", "#bf702d")) + scale_fill_manual(values = c("#003741", "#bf702d"))  + ylim(0, 1) + 
    labs(x = "Normalised titre", y = "Relative risk of protection", color = "Biomarker", fill = "Biomarker") + 
    scale_x_continuous(
        name = "log10 Titre value (spike)", 
        breaks = seq(0, 1, length.out = 10), labels = labels_x_spike,
        sec.axis = sec_axis(~ . * 1, 
    ...
    ggtitle("Fitted curves for rCOP")


pCOP <- p1 / (p2 + p3) + plot_layout(guides = "collect") + 
    plot_layout(width = c(1, 1)) & theme(text = element_text(size = 15, color = "black"))



 

(pA / pBC) | pCOP + plot_annotation(tag_levels = "A") + plot_layout(width = c(2, 1, 0)) & theme(text = element_text(size = 15, color = "black")) 
