...
run_uncert_model_bi <- function(modeli) {
    ...
    dir.create(here::here("outputs", "sim_data", modeli$name), showWarnings = FALSE)
    saveRDS(res, file = here::here("outputs", "sim_data", modeli$name, paste0("sim_data_N_", N, ".rds")))
}


plot_sim <- function(modeli) {
  modeli <- modelcesCOP[[7]]
  uncert <- 0.1
  ...
  #plot_obs_biomarkers_paired_sample(res$observed_biomarker_states)
    ...
    ggsave(here::here("outputs", "sim_data", modeli$name, "figs", paste0("N_", round(N, 1)), "simulated_summary.png"), height = 15, width = 15)
...
pop_size <- c(10, 20, 50, 100, 200, 500, 1000)

simpar_func <-  function(N) {
  list(
    N = N,
    T = T,
    S = 5,
...
observation_times_ces <- function(simpar) {
  1:simpar$N %>% 
...
observation_times_ppes <- function(simpar) {
  1:simpar$N %>% 
    map_df( ~tibble(i=.x, t = floor(
...
modelcesNoCOP <- map(pop_size, 
  function(N) {
    simpar <- simpar_func(N)

    list(
      name = "cesNoCOP_notd",
      observation_times = observation_times_ces(simpar),
      protection_curve = 0,
      simpar = simpar,
      uncert = 0.1
    ) %>% get_foe_par
...
modelcesNoCOP %>% map(~run_uncert_model_bi(.x) )
modelcesCOP %>% map(~run_uncert_model_bi(.x) )


saveRDS(modelcesNoCOP, here::here("outputs", "sim_data", modelcesNoCOP[[1]]$name, "inputs_N.RDS"))
saveRDS(modelcesCOP, here::here("outputs", "sim_data", modelcesCOP[[1]]$name, "inputs_N.RDS"))
