...
modelcesNoCOP <- readRDS(here::here("outputs", "sim_data", "cesNoCOP_notd", "inputs.RDS"))
modelcesCOP <- readRDS(here::here("outputs", "sim_data", "cesCOP_notd", "inputs.RDS"))
modelppesNoCOP <- readRDS(here::here("outputs", "sim_data", "ppesNoCOP_notd", "inputs.RDS"))
modelppesCOP <- readRDS(here::here("outputs", "sim_data", "ppesCOP_notd", "inputs.RDS"))
...
p1A <- res$observed_biomarker_states %>% mutate(i = factor(i, levels = rev(ids_order))) %>%
  ggplot() + 
    geom_tile(data = start_bleed, aes(y = i, x = t / 2, width = t ), alpha = 0.9, fill = "gray90") +
    geom_tile(data = end_bleed, aes(y = i, x = t + (T - t) / 2, width = (T - t) ), alpha = 0.9, fill = "gray90") +
    geom_point(aes(x = t, y = i), shape = "|", size = 2) + 
    geom_point(data = epi_times_sims, aes(x = t, y = i, color = exp_type),  shape = 15) + theme_bw() + 
    labs(x = "Time in study (days)", y = "Participants", color = "Exposure type") + 
    scale_color_manual(values = c("#882255", "#DDCC77")) + 
    geom_hline(yintercept = seq(0.5, N + 0.5, 1), color = "gray90") +
    theme(panel.grid.major.y = element_blank(), axis.text.y = element_blank(), legend.position = "top") + 
    ggtitle("Overview of serological survey and \nunderlying epidemic for simulated CES data") 



p1B <- epi_times_sims %>% 
        ggplot() + 
            geom_density(aes(x = t, y = ..count.., fill = exp_type), size = 2, alpha = 0.5) + 
            geom_rug(aes(t, y = NULL, color = exp_type)) +
            theme_bw()  + 
            xlim(0, 120) + 
            scale_color_manual(values = c("#882255", "#DDCC77")) +
            guides(color = "none") + 
            ...
            labs(x = "Time in study (days)", y = "Count", fill = "") + 
            ggtitle("Exposure-infection timings")

p1 <- p1A / p1B + plot_layout(heights = c(3, 1))
...
p2 <- data.frame(start_model = res$start_titre) %>% 
  ...
  labs(x = "Titre at start of study", y = "Density") + theme_bw() +
  ggtitle("Distribution of initial titre values, y")
...
p4 <- traj_true %>% 
  ggplot() + 
   ...
   # geom_segment(x = 7, xend = 70, y = 2, yend = 1.15, color = "blue") + 
    ...
    ggtitle("Antibody kinetics post-infection", expression(f(s)))


## PLOT E: Titre dependent boosting

p5 <- p2 / p3 / p4 + plot_layout(heights = c(1, 1, 2))   
(p1 | (p5 )) + plot_layout(width = c(1, 1)) + plot_annotation(tag_levels = "A") 
ggsave(here::here("outputs", "sim_data", "summary_fig_A_CES.pdf"), height = 12, width = 12)
