...
#'
#' @export
rjmc_func_pp <- function(model, data, settings, par = NULL) {
  settings <- check_settings_full(settings)
  if (length(par) == 0) {
    par <- rep(list(list(type = "None")), settings[["numberChainRuns"]])
    output <- get_output_pp(model, data, settings, FALSE, par)
  } else {
    output <- get_output_pp(model, data, settings, TRUE, par)
  }
  output
}

get_output_pp <- function(model, data_list, settings, update_ind, par) {
  ...
  if (settings[["runParallel"]]) {
    out_raw <- mclapply(1:settings[["numberChainRuns"]], 
      function(i) {
        run_rjmc_pp(model, data_list, settings, update_ind, par[[i]], i)
      },
      mc.cores = settings[["numberCores"]]
    )
  } else {
    for (i in 1:settings[["numberChainRuns"]]) {
      out_raw[[i]] <- run_rjmc_pp(model, data_list, settings, update_ind, par[[i]], i)
    }
  }
...
check_settings_full <- function(settings) {
