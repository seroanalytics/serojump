...
namespace rjmc_full{
    ...
    struct RJMC_FULL_D
    {
        ...
        List currentParsCOP, proposalParsCOP, currentParsObs, currentParsAb;
        StringVector exposureNames;
        StringVector fittedParamNames;

        // call each step
        void updateParams_init(NumericVector paramsN) {
            ...
            for (int i = 0; i < this->B; ++i) {
                ...
                this->proposalParsCOP[this->biomarkers[i]] = currentParsCOP_i;
            }
            for (int i = 0; i < this->B; ++i) {
        ...
        double evalLogPosterior(const VectorXd& param, const VectorXd& jump, const VectorXd& jumpInf, const MatrixXd& covariance, const RObject& dataList, bool init = false)
        {
          ...
          //  this->updateLists();
            ...
            for (int i = 0; i < this->N; i++) {
                if (jump[i] > -1) {
                    logLikelihood_time += this->exposureFunctionDensity(jump[i]);
                }
            }
            ...
            double long logPriorInf;
            double long logPriorExp;
            double long logPriorExpInf;
        ...
        void recalculateTitreAll(VectorXd jump_inf) {
            std::vector<DoubleWithString> df_order_exp_i;
            for (int i_idx = 0; i_idx < this->N; i_idx++) {
                df_order_exp_i = proposalEventsFull[i_idx];
                this->proposalTitreFull[i_idx] = this->calculateTitre(df_order_exp_i, i_idx, jump_inf);
            }
        }

        std::vector<std::vector<DoubleWithString> > calculateTitre(std::vector<DoubleWithString>& orderedEvents, int i_idx, VectorXd jump_inf)  {
            ...
            for (int b = 0; b < this->B; b++) {
                ...
                for (int i = 1; i < orderedEvents.size(); i++) {
                    ...
                    if (orderedEvents[i].name != "bleed") {
                        if (orderedEvents[i].name == this->exposureFitted){
                            if (jump_inf[i_idx] == 1) {
                            ...
                            NumericVector known_inf_i = this->knownInf[orderedEvents[i].name]; 
                            if (known_inf_i[i_idx] > -1) {
                                anchor_titre = titre_obs;
                                anchor_time = orderedEvents[i].value;
                                anchor_func = orderedEvents[i].name;  
    ...
    //  /       this->posteriorObsTitre = Array::Zero((this->posteriorSamplesLength, this->lengthJumpVec, this->B);
            ...
            this->updateParams_init(paramsN);
            this->define_abs();
            ...
            for (int i_idx = 0; i_idx < this->N; i_idx++) {
                ...
                df_order_titre_i = this->calculateTitre(df_order_exp_i, i_idx, this->currentInf);
            ...
            while(isinf(initialLogLikelihood) || isnan(initialLogLikelihood)){
                ...
                this->updateParams_init(paramsN);
                this->define_abs();
                this->recalculateTitreAll(this->currentInf);
                initialLogLikelihood = this->evalLogPosterior(initialSample, initialJump, initialInf, this->currentCovarianceMatrix, this->dataList, true);
            }
       ...
       //     this->posteriorTitreExp = MatrixXd::Zero(this->posteriorSamplesLength, this->lengthJumpVec);
        ...
        {        
           ...
           // Evaluate the ab kinetics parameters and do rjmcmc
            ...
            this->define_abs();
            this->recalculateTitreAll(this->proposalInf);
        ...
        void bookKeepingSizeInit() {
            int count_exp = 0;
            int count_inf = 0;
            for (int i = 0; i < this->N; i++) {
                if (this->currentJump[i] > -1) {
                    count_exp++;
                    if (this->currentInf[i] == 1) {
                        count_inf++;
                    }
                }
            }            
            this->currInferredExpN = count_exp;
            this->currInferredInfN = count_inf;
        }
