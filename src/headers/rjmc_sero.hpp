    ...
    struct RJMC_SERO_D
    {
        ...
        void JumpProposalDist() {
            ...
            // Stay same
               ...
               // Rcpp::Rcout << "In stay same" << std::endl;
                ...
                if(this->currInferredInfN != this->knownInfsN) {
                    ...
                    // New sample
                    if (r < 0.05) {
                        this->proposalJump(this->currJumpIdx) = this->exposureFunctionSample(); // this->exposureFunctionSample(); //runif(1, 1, 150) + 121 //
                        while ((this->proposalJump(this->currJumpIdx) >= this->endTitreTime(this->currJumpIdx) - 7) | (this->proposalJump(this->currJumpIdx) < this->initialTitreTime(this->currJumpIdx))) {
                            this->proposalJump(this->currJumpIdx) = this->exposureFunctionSample(); // this->exposureFunctionSample(); //runif(1, 1, 150) + 121 //
                        }
                    // Update existing sample
                    } else {
                        double temp = this->currentJump(this->currJumpIdx) + normalDistSample(0, exp(this->adaptiveGibbsSD(this->currJumpIdx)));
                        if (temp >= this->endTitreTime(this->currJumpIdx) - 7) {
                            temp = this->endTitreTime(this->currJumpIdx) - 8; // this->exposureFunctionSample(); //runif(1, 1, 150) + 121 //
                        } else if (temp < this->initialTitreTime(this->currJumpIdx)) {
                            temp = this->initialTitreTime(this->currJumpIdx); // this->exposureFunctionSample(); //runif(1, 1, 150) + 121 //
                        }
                        this->proposalJump(this->currJumpIdx) = temp;
                    }
                }
                this->currJumpType = 1;
            // Add an infection same
              ...
              //  Rcpp::Rcout << "In addition same" << std::endl;
                ...
                r = uniformContinuousDist(0, 1);
                if ((this->historicJump(this->currJumpIdx) < 0) | (r < 0.05)) {
                        this->proposalJump(this->currJumpIdx) = this->exposureFunctionSample(); // this->exposureFunctionSample(); //runif(1, 1, 150) + 121 //
                        while ((this->proposalJump(this->currJumpIdx) >= this->endTitreTime(this->currJumpIdx) - 7) | (this->proposalJump(this->currJumpIdx) < this->initialTitreTime(this->currJumpIdx))) {
                            this->proposalJump(this->currJumpIdx) = this->exposureFunctionSample(); // this->exposureFunctionSample(); //runif(1, 1, 150) + 121 //
                        }
                } else {
                    this->proposalJump(this->currJumpIdx) = this->historicJump(this->currJumpIdx);
                }
                this->currJumpType = 2;
                this->propInferredInfN = this->currInferredInfN + 1;
            }
        ...
        void updateGibbsTiming() {
            ...
            for (int i = 0; i < this->noGibbsSteps; i++) { 
                ...
                if (r < 0.05) {
                    this->proposalJump(this->gibbsIdx) = this->exposureFunctionSample(); // this->exposureFunctionSample(); //runif(1, 1, 150) + 121 //
                    while ((this->proposalJump(this->gibbsIdx) >= this->endTitreTime(this->gibbsIdx) - 7) | (this->proposalJump(this->gibbsIdx) < this->initialTitreTime(this->gibbsIdx))) {
                        this->proposalJump(this->gibbsIdx) = this->exposureFunctionSample(); // this->exposureFunctionSample(); //runif(1, 1, 150) + 121 //
                    }
                // Update existing sample
                } else {
                    double temp = this->currentJump(this->gibbsIdx) + normalDistSample(0, exp(this->adaptiveGibbsSD(this->gibbsIdx)));
                    if (temp >= this->endTitreTime(this->gibbsIdx) - 7) {
                        temp = this->endTitreTime(this->gibbsIdx) - 8; // this->exposureFunctionSample(); //runif(1, 1, 150) + 121 //
                    } else if (temp < this->initialTitreTime(this->gibbsIdx)) {
                        temp = this->initialTitreTime(this->gibbsIdx); // this->exposureFunctionSample(); //runif(1, 1, 150) + 121 //
                    }
                    this->proposalJump(this->gibbsIdx) = temp;
                }
