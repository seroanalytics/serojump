...
p2 <- outputfull_2$sim_states %>% filter(exp_ind_sim == 1)  %>% mutate(inf_state = case_when(inf_ind_sim==0~"Not infected", inf_ind_sim==1~"Infected")) %>%
    ggplot() + geom_point(aes(x = init_titre, y = inf_ind_sim, fill = inf_state), alpha = 0.8, size = 2, shape = 21) +
        ...
        geom_smooth(aes(x = init_titre, y = inf_ind_sim, color = "black" )) + theme_bw() + 
        scale_color_manual(values = c("red" = "red", "black" = "black"), labels = c("Functional simulated curve", "Realised simualted curve")) + 
        labs(x = expression("Pre-infection titre, Z"[j]^0), y = "Probability of infection", fill = "Simulated infection state") + 
...
runsaveRJMCMC( "inferExp", "cesCOP", "0.3", 0, evaluateLogLikelihood, par_tab, exp_prior, cop_func)
...
runsaveRJMCMC( "inferExp", "cesNoCOP", "0.1", 0, evaluateLogLikelihood, par_tab, exp_prior, cop_func)
runsaveRJMCMC( "inferExp", "cesNoCOP", "0.3", 0, evaluateLogLikelihood, par_tab, exp_prior, cop_func)
...
runsaveRJMCMC( "knownExp", "cesCOP", "0.1", 0, evaluateLogLikelihood, par_tab, exp_prior, cop_func, TRUE)
runsaveRJMCMC( "knownExp", "cesCOP", "0.3", 0, evaluateLogLikelihood, par_tab, exp_prior, cop_func, TRUE)
...
runsaveRJMCMC( "knownExp", "cesNoCOP", "0.1", 0, evaluateLogLikelihood, par_tab, exp_prior, cop_func, TRUE)
runsaveRJMCMC( "knownExp", "cesNoCOP", "0.3", 0, evaluateLogLikelihood, par_tab, exp_prior, cop_func, TRUE)
