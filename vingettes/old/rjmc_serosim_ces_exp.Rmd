...
b0_rep <- runif(1000, -10, 10)
b1_rep <- runif(1000, -10, 10)
...
library(ggdist)
lol %>% ggplot() + 
    stat_lineribbon(aes(x = t, y = vals))
...
runsaveRJMCMC <- function(modelname, obs_er, prob_known, evaluateLogLikelihood, par_tab, exp_prior, cop_func, known_exp = FALSE) {
    # settings used for the ptmc model 
    data_t <- clean_simulated_rjmcmc(modelname, obs_er, prob_known, known_exp)
    ...
    saveRDS(fitfull, here::here("outputs", "sim_full", modelname, paste0("rjmc_fit_pr_", "obs_", obs_er, ".RDS")))
}
...
runsaveRJMCMC( "ces_exp_only", "0", 0, evaluateLogLikelihood, par_tab, exp_prior, cop_func, TRUE)
post_exp_inf_full <- postprocess_run( "ces_exp_only", "ces", "0", 4)

#runsaveRJMCMC( "ces", "0", 0, evaluateLogLikelihood, par_tab, exp_prior, cop_func, FALSE)
#postprocess_run( "ces", "0", 4)
...
pushover(message = "RJMCMC simulator finished!") 
...
postprocess_run <- function(modelname, modelname_sim, obs_er, n_chains) {
    ...
    modelname <- "ces_exp_only"
    modelname_sim <- "ces"
    obs_er <- "0"
    n_chains <- 4

    library(ggdist)
    dir.create(here::here("outputs", "sim_full", modelname, "figs"))
    dir.create(here::here("outputs", "sim_full", modelname, "figs", paste0("obs_", obs_er)))
    modelA <- readRDS(here::here("outputs", "sim", modelname_sim, "inputs.RDS"))
    res <- readRDS(file = here::here("outputs", "sim", modelname_sim, paste0("sim_data_", obs_er, ".rds")))
    fitfull <- readRDS(here::here("outputs", "sim_full", modelname, paste0("rjmc_fit_pr_", "obs_", obs_er, ".RDS")))
    post <- fitfull$post
...
outputfull <- postprocess_run( "ces_exp_only", "ces", "0", 4)
...
```{r}
    ...
    add_inferred_inf <- function(df, cutoff) {
        df %>% mutate(infer_exp = case_when(prop_exp < cutoff ~ 0, prop_exp >= cutoff ~ 1))
    }
    add_uncertainty <- function(df, no_sample) {
        df %>% mutate(id = as.numeric(id)) %>% mutate(prop_exp = samples_inf / no_sample, 
        ub = prop_exp + 1.98 * sqrt(prop_exp * (1-prop_exp) / no_sample),
        lb = prop_exp - 1.98 * sqrt(prop_exp * (1-prop_exp) / no_sample),
        )
    } 
    ...
    post_jump_chain_inf <- map_df(1:n_chains,
        function(x) {
            post_jump_chain <- post$jump[[x]]
            data.frame(
                chain = x,
                id = as.character(1:N),
                samples_inf = 1:N %>% map( ~(post_jump_chain[, .x] != -1) %>% sum ) %>% unlist
            )
        }
    ...
    size_missed <- post_jump_pooled_inf$prop_exp 
    ...
    saveRDS(list(jumpmcmc = jump_post_combined, jump_noninf = jump_format_notinferable, jump_infer_processed = jump_format_inf, jump_all_processed = jump_format_all), 
        file = here::here("outputs", "sim_full", modelname, "figs", paste0("obs_", obs_er), "processed.RDS"))
...
ggsave(here::here("outputs", "sim", modelname, "figs", paste0("obs_", obs_er), "ab_kinetics_recov.png"), height = 10, width = 10)


```

# PART 2: EXPOSURE RECOVERY
...
fit_states %>% summarise(exp_tot = sum(exp_ind), .by = c(sample)) %>% 
    ggplot() + geom_histogram(aes(exp_tot))

# Timining of exposures 
fit_states %>% filter(exp_ind == 1) %>% summarise(exp_time_mean = mean(exp_time), .by = c(id)) %>% 
    ggplot() + geom_histogram(aes(exp_time_mean))

# Probability of exposure per individual 
fit_states %>% summarise(exp_ind_prop = mean(exp_ind), .by = c(id))
...
left_join(
    sim_states %>% select(id, inf_ind_sim),
    fit_states %>% select(id, inf_ind) %>% summarise(inf_post = mean(inf_ind), .by = "id")
) %>% left_join(df_data) %>% 
    ggplot() + 
        geom_point(aes(x = start_titre, y = inf_post, color = as.character(inf_ind_sim)))

# b) Infection given exposure 

left_join(
    sim_states %>% filter(exp_ind_sim == 1) %>% select(id, inf_ind_sim),
...
df_cop_full <- fit_states %>% filter(exp_ind == 1) %>% left_join(df_data) %>% filter(sample %in% 1:2000)
#df_cop_sum <- post_exp_inf_full %>% filter(exp_ind == 1) %>% summarise(inf_ind_prop = mean(inf_ind), .by = c(id)) %>% left_join(df_data)

require(rstanarm)
df_post1 <- map_df(1:2000, 
        ~(glm(inf_ind ~ start_titre, data = df_cop_full %>% filter(sample == .x), family = binomial(link = "logit"))$coefficients %>% as.data.frame %>% t %>% as.data.frame)
)

pr_switch <- function(x, ests, exp) pmin(plogis(ests[1] + ests[2] * x), 1)

df_traj <- 1:2000 %>% map_df(
    ~data.frame(
    sample = .x,
    titre = seq(0, 4, 0.1),
...
df_cop_full %>%    
    ggplot() + 
        ...
        geom_ribbon(data = df_traj, aes(x = titre, ymin = .lower, ymax = .upper), alpha = 0.3) +
             +
        ylim(0, 1)
...
(p1 + p2) / (p3 + p4) / p5 
ggsave(here::here("outputs", "sim", "ces", "figs", paste0("obs_", obs_er), "epi_recov.png"))
...
ggsave(here::here("outputs", "sim", "ces", "figs", paste0("obs_", obs_er), "epi_recov_nice.png"), height = 12)
