...
gambia_pvnt_w2 <- get_data_titre_model_wave2() %>% mutate(biomarker = "neut_delta") # This is the empirical prior for the exposure time
gambia_exp_w2 <- get_exposures_wave2() # This is the empirical prior for the exposure time
exp_prior_w2 <- get_exp_prior_wave2() # PLEASE CHANGE, This is an example of the emprical prior need
...
obsLogLikelihood = function(ll, titre_val, titre_est, pars) {
    LOD <- # add limited of detection here
    if (titre_val <= LOD) {
        ll <- ll + pcauchy(LOD, titre_est, pars[1], log.p = TRUE)
    } else {
...
biomakers <- c("neut_delta")
exposureType <- c("delta", "vax")

# Define the observational model
observationalModel <- list(
    names = c("neut_delta"),
    model = makeModel(addObservationalModel("neut_delta", c("sigma"), obsLogLikelihood)),
    prior = add_par_df("sigma", 0.0001, 4, "unif", 0.0001, 4) # observational model,
)

# Define the antibody kinetics model
abkineticsModel <- list(
    names = c("none", "delta", "vax"),
    model = makeModel(
            addAbkineticsModel("neut_delta", "none", FALSE,  c("wane"), noInfSerumKinetics),
            addAbkineticsModel("neut_delta", "delta", TRUE, c("a_d", "b_d", "c_d"), infSerumKinetics),
            addAbkineticsModel("neut_delta", "vax", FALSE, c("a_vax", "b_vax", "c_vax"), infSerumKinetics)
        ),
    prior = bind_rows(
...
copModel <- list( 
        names = c("IgG"),
        model = makeModel(addCopModel("neut_delta", "delta", c("beta0", "beta1"), copLogLikelihood)),
        prior = bind_rows(
...
# Run the model 
runRJMCMC(modelW2, settings, "test/crick", "delta_wave")
