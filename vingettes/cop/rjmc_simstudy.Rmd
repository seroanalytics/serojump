...
# NOT THIS ONE
runSimulatedRJMCMC <- function(modeldefinition, settings, filename, sim_data, obs_er) {
    # Gets the simulated data from serosim package
    data_titre_model <- clean_simulated_rjmcmc(sim_data, obs_er, 0, known_exp = FALSE) %>% rename(IgG = titre)

    # obs_er is the observation error
    modelname <- paste0("obs_", obs_er)
    # Create the model
    model_sim <- createSeroJumpModel(data_titre_model, data_known, modeldefinition)
    runRJMCMC(model_sim, settings, filename, modelname)
}

runSimulatedRJMCMC(modeldefinition, settings, "local/sim_study/cesCOP_notd/InferExp", "cesCOP_notd", "0.1")
runSimulatedRJMCMC(modeldefinition,settings, "local/sim_study/cesCOP_notd/InferExp", "cesCOP_notd", "0.3")
runSimulatedRJMCMC(modeldefinition,settings, "local/sim_study/cesCOP_notd/InferExp", "cesCOP_notd", "0.5")

runSimulatedRJMCMC(modeldefinition, settings, "local/sim_study/cesNoCOP_notd/InferExp", "cesCOP_notd", "0.1")
runSimulatedRJMCMC(modeldefinition, settings, "local/sim_study/cesNoCOP_notd/InferExp", "cesCOP_notd", "0.3")
runSimulatedRJMCMC(modeldefinition, settings, "local/sim_study/cesNoCOP_notd/InferExp", "cesCOP_notd", "0.5")
...
modeldefinition, settings, "local/sim_study/cesCOP_notd/InferExp", "cesCOP_notd", "0.1"
sim_data <- "cesCOP_notd"
obs_er <-  "0.1"

runSimulatedRJMCMC <- function(modeldefinition, settings, filename, sim_data, obs_er) {
    data_titre_model <- clean_simulated_rjmcmc(sim_data, obs_er, 0, known_exp = TRUE) %>% rename(IgG = titre)
    ...
    modelname <- paste0("obs_", obs_er)
    model_sim <- createSeroJumpModel(data_titre_model, data_known, modeldefinition, known_exp = known_exp)
