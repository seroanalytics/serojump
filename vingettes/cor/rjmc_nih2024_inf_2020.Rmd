...
data_exp <- get_data_titre_nih_prior2022(2020, "H1")
...
biomarkers <- c("A/Brisbane/02/2018", "A/Brisbane/02/2018e")
exposureTypes <- c("vax", "h1_2020")
exposureFitted <- c("h1_2020")

# Define the observational model
observationalModel <- list(
    model = makeModel(
        addObservationalModel("A/Brisbane/02/2018", c("sigma"), obsLogLikelihood),
        addObservationalModel("A/Brisbane/02/2018e", c("sigma_e"), obsLogLikelihood)
        ),
    prior = bind_rows(
...
abkineticsModel <- list(
    model = makeModel(
            addAbkineticsModel("vax", "A/Brisbane/02/2018", "vax", c("a_vax", "b_vax", "c_vax"), infSerumKinetics),
            addAbkineticsModel("h1_2020","A/Brisbane/02/2018", "h1_2020", c("a_v", "b_v", "c_v"), infSerumKinetics),
            addAbkineticsModel("vax_e", "A/Sydney/5/2021e", "vax", c("a_vax_e", "b_vax_e", "c_vax_e"), infSerumKinetics),
            addAbkineticsModel("h1_2020_e", "A/Sydney/5/2021e", "h1_2020", c("a_v_e", "b_v_e", "c_v_e"), infSerumKinetics)
        ),
    prior = bind_rows(
        ...
        add_par_df("a_v", -5, 6, "norm",  0, 2), # ab kinetics
...
inf_prior_3 <- function(N, E, I, K) {
    N_adj <- N - K
    E_adj <- E - K
    logPriorExpInf <- lfactorial(E_adj) + lfactorial(N_adj - E_adj) - lfactorial(N_adj ) + dbinom(E_adj, N_adj, 0.0001, log = TRUE)
    logPriorExpInf
}
...
#runInfRJMCMC(seroModel_nih_p1, settings, "local/nih_2020_inf/prior1", "h1")
#postprocessFigsInf("local/nih_2024_inf/prior1", "h1", 4)
# Getting suck at high values of infection ??

#runInfRJMCMC(seroModel_nih_p2, settings, "local/nih_2020_inf/prior2", "h1")
#require(bayesplot)
#postprocessFigsInf("local/nih_2024_inf/prior2", "h3", 4)


runInfRJMCMC(seroModel_nih_p3, settings, "local/nih_2020_inf/prior3", "h1")
postprocessFigsInf("local/nih_2024_inf/prior3", "h1", 4)
