...
copFuncFormGeneralised <- function(esttitreExp, gamma, k, beta, beta_z, betz_sigma, alpha, alpha_z, alpha_sigma ) {
...
require(boot)
N <- 1000
gamma_vec <- runif(N, 0.2, 1) # thing dividing the (no fixed effects) (0, 1)
...
df_summary <- map_df(1:N,
    function(i) {
        data.frame(
            samle = i,
            x = seq(0, 10, 0.1),
            f = map_dbl(seq(0, 10, 0.1), copFuncFormGeneralised, gamma = gamma_vec[i], k = k_vec[i], beta = beta_vec[i], alpha = alpha_vec[i]) 
        ) 
    }
) %>% group_by(x) %>% mean_qi(f, .width = c(0.1, 0.5, 0.75, 0.95))
