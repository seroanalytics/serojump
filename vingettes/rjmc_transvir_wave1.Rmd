...
gambia_meta %>% filter(vaccinated == "Yes")
##
...
gambia_pvnt_w1 <- get_data_titre_model_wave1() # This is the empirical prior for the exposure time
gambia_exp_w1 <- get_exposures_wave1() # This is the empirical prior for the exposure time
...
par_tab <- bind_rows( 
    add_par_df("a", 0.1, 2, "norm",  1, 0.5), # ab kinetics    
    add_par_df("b", 0, 1, "norm",  0.3, 0.05), # ab kinetics         
    add_par_df("c", 0.5, 4, "unif", 0.5,  4), # ab kinetics       
    add_par_df("sigma_obs", 0.01, 5, "exp", 1, NA), # observational model
...
ids_inf <- fit_states_exp_prob %>% filter(exp_post > 0.6) %>% pull(id)
...
fit_states_exp_prob %>% left_join(df_data_alt) %>% filter(id %in% ids_inf) %>%
    ggplot() + 
...
post <- fitfull$post
mcmc_mean <- post$mcmc %>% combine %>% as.data.frame %>% mutate(c = c_slope) %>%
