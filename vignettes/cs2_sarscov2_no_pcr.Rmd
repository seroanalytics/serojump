...
save_info_w2 <- list(
    ...
    model_name = "wave_2_no_pcr_non"
)

save_info_w3 <- list(
    file_name = "transvir_data",
    model_name = "wave_3_no_pcr_non"
)
...
N <- model_summary_w2$fit$data$N
recover_serojump <- model_summary_w2$post$fit_states %>% group_by(id) %>% filter(inf_time > -1) %>% summarise(n = n() / 4000) %>% 
...
(p1 + p2 + p3) / (p4 + p5) / p6 + plot_layout(guides = "collect", heights = c(1,1,2))  + 
    labs(x = "Biomarker used for serological detection", y = "Sensitivity", title = "Sensitivity of serological detection")
